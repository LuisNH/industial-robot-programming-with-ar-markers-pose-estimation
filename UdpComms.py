...
class UdpComms:
    def __init__(self, udpIP, portTX, portRX, enableRX=False, suppressWarnings=True):
        """
        Constructor
        :param udpServerIP: Must be string e.g. "127.0.0.1"
        ...
        self.udpServerIP = udpIP
        self.udpClientIP = None
    ...
    def SendData(self, strToSend):
        ...
        print(self.udpClientIP)

        self.udpSock.sendto(
            bytes(strToSend, "utf-8"), (self.udpClientIP, self.udpSendPort)
        )
    ...
    def ReceiveData(self):
        ...
        data = None
        try:
            data, address = self.udpSock.recvfrom(65535)
            self.udpClientIP = address[0]
